...
function setView(view) {
  ...
  const hideDashboardWidgets = view === 'stock' || view === 'users';
...
function renderEquipmentProfile(profile) {
  ...
  $('#equipmentProfileBody').innerHTML = `
    ...
    <div><span>Etiqueta TIMSA</span><strong><button class="ghost" type="button" onclick="printTimsaLabel('${escapeHtml(item.id)}')">Imprimir etiqueta</button></strong></div>
  `;
  ...
  $('#equipmentMaintenanceList').innerHTML = (profile.maintenance || []).map((entry) => `
    <div><strong>${escapeHtml(entry.event_type)}</strong><span>${escapeHtml(formatDate(entry.created_at))} &middot; ${escapeHtml(entry.changed_by || 'Sistema')}</span></div>
  `).join('') || '<p class="empty-module">Sin historial.</p>';
}
...
function printTimsaLabel(equipmentId) {
  const item = state.items.find((entry) => entry.id === equipmentId);
  if (!item) return;
  ...
  printWindow.document.write(`
    ...
    <html>
    <head>
      <title>Etiqueta TIMSA - ${escapeHtml(item.serial_number)}</title>
      <style>
        @page { margin: 0; size: 102mm 152mm; }
    ...
    <body>
      <div class="timsa-label">
        <div class="timsa-header">
          <h1>HUTCHISON PORTS</h1>
          <h2>TIMSA</h2>
          <div class="subtitle">Sistema de Administracion de Tecnologia de Informacion</div>
        </div>
        <div class="timsa-body">
          <div class="timsa-field">
            <span class="label">No. de Serie</span>
            <span class="value value-large">${escapeHtml(item.serial_number)}</span>
          </div>
          <div class="timsa-field">
            <span class="label">ID de Inventario</span>
            <span class="value">${escapeHtml(item.asset_tag || 'N/A')}</span>
          </div>
          <div class="timsa-field">
            <span class="label">Equipo</span>
            <span class="value">${escapeHtml(item.equipment_type)} - ${escapeHtml(item.brand)} ${escapeHtml(item.model)}</span>
          </div>
          <div class="timsa-field">
            <span class="label">Usuario Asignado</span>
            <span class="value">${escapeHtml(item.assigned_user || 'Sin asignar')}</span>
          </div>
          <div class="timsa-field">
            <span class="label">Ubicacion / Area</span>
            <span class="value">${escapeHtml(item.location)} / ${escapeHtml(item.area)}</span>
          </div>
          <div class="timsa-barcode-placeholder">
            ${escapeHtml(item.serial_number)} | ${escapeHtml(item.asset_tag || 'N/A')}
          </div>
        </div>
        <div class="timsa-footer">
          <strong>SATI-TIMSA</strong> &middot; Activo registrado el ${escapeHtml(formatDate(item.created_at))} &middot; www.timsa.com.mx
        </div>
      </div>
